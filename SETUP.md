...
사이트를 하나로 고정하지 않습니다. **팀마다 자기 사이트에 저장**됩니다.

| 탭 위치 | 저장 위치 | 설정 |
| --- | --- | --- |
| **팀 채널** | 그 팀의 SharePoint 사이트 | **불필요** |
| 그룹 채팅 | `VITE_SHAREPOINT_SITE_PATH` 의 공용 사이트 | 필요 |
| 개인 탭 | 〃 | 필요 |

전사 배포에 이 구조가 맞습니다.

- 팀이 100개면 사이트 100곳에 나뉘어 저장됩니다. 한 사이트에 몰리지 않습니다.
- **권한이 팀 경계를 그대로 따릅니다.** 다른 팀 사이트에 접근 권한이 없으면
  그 팀 회의 데이터도 보이지 않습니다. 앱이 따로 격리 로직을 만들 필요가 없습니다.
- 팀마다 사이트를 미리 만들 필요가 없습니다. 팀에는 이미 사이트가 딸려 있고,
  목록 4개는 첫 사용 시 앱이 자동 생성합니다.

### 공용 사이트는 직접 만들면 됩니다

사이트 생성은 보통 **일반 사용자도 가능**합니다. 관리자에게 요청할 필요가 없습니다.
전용 팀을 하나 만들어 그 팀의 사이트를 저장소로 쓰는 방식이 간편합니다.

```text
Teams → 팀 만들기       → 사이트가 함께 생성됨
또는 SharePoint → 사이트 만들기
```

목록 4개는 앱이 첫 사용 시 자동 생성하므로 미리 만들 필요가 없습니다.

### 접근 권한이 곧 사용 범위입니다

**이 사이트에 접근할 수 있는 사람만 채팅에서 앱을 쓸 수 있습니다.**
권한이 없으면 채팅 탭에서 "공용 저장소 사이트에 접근할 수 없습니다" 오류가 뜹니다.

| 사이트 접근 범위 | 채팅에서 쓸 수 있는 사람 |
| --- | --- |
| 팀 구성원만 (기본값) | 그 팀 구성원 |
| 특정 부서 추가 | 그 부서까지 |
| 모든 사용자(외부 제외) | 전사 |

**전용 팀을 저장소로 쓸 때** — 팀 사이트는 기본적으로 팀 구성원만 접근합니다.
전사에서 채팅 기능을 쓰게 하려면 셋 중 하나입니다.

1. **팀을 공개(Public)로 만들기** ← 가장 간단
   조직 내 누구나 스스로 검색해 참가할 수 있습니다. 관리자도, 초대도 필요 없습니다.
   앱이 채팅 탭에서 가입 방법을 안내하므로 참석자가 직접 해결합니다.
2. 필요한 사람을 그 팀에 초대 (인원이 적을 때)
3. SharePoint 사이트 권한에 **모든 사용자(외부 사용자 제외)** 직접 추가
   (팀 멤버십과 무관하게 사이트만 열기)

1번이면 추가 작업이 사실상 없습니다.

> 유의 · 공개 팀은 **누구나 가입할 수 있고, 가입하면 그 사이트에 저장된 모든 채팅의
> 회의명·참석자·시간을 SharePoint 에서 직접 볼 수 있습니다.**
> 민감한 회의는 팀 채널에서 쓰세요. 채널은 그 팀 사이트를 쓰므로 팀 밖으로 새지 않습니다.

> 팀 채널에서 쓰는 경우에는 이 설정이 **전혀 관여하지 않습니다.**
> 각 채널은 자기 팀 사이트를 쓰므로 별도 권한 작업이 필요 없습니다.

### 채팅에서 쓸 때 주의

공용 사이트에 접근 가능한 사람은 **그 사이트에 저장된 모든 채팅의 회의 정보를 볼 수 있습니다.**
앱 화면에서는 채팅별로 걸러 보여주지만, SharePoint 목록을 직접 열면 전부 보입니다.
SharePoint 권한이 목록 단위라 회의별로 쪼갤 수 없기 때문입니다.

팀 채널은 팀 사이트를 쓰므로 이 문제가 없습니다. **민감한 회의는 채널에서 쓰세요.**

### 공용 사이트는 폴백입니다

`VITE_SHAREPOINT_SITE_PATH` 를 지정해도 **팀 채널 탭은 여전히 팀 사이트를 씁니다.**
이 값은 딸린 사이트가 없는 그룹 채팅·개인 탭에서만 쓰입니다.

```bash
VITE_SHAREPOINT_SITE_PATH=/sites/<사이트이름>
```

전체 URL 이 아니라 **`/sites/...` 경로만** 넣습니다.
회의는 채팅별로 격리되므로 여러 채팅이 한 사이트를 공유해도 섞이지 않습니다.

### 채널 전용으로만 쓴다면

`VITE_SHAREPOINT_SITE_PATH` 를 **비워두세요.** 설정이 필요 없고, 팀이 늘어나도
추가 작업 없이 자동으로 확장됩니다.
