...
```text
SharePoint → 사이트 만들기 → 팀 사이트
```

만든 뒤 **누가 이 사이트에 접근할 수 있는지**가 곧 "누가 채팅에서 이 앱을 쓸 수 있는지"가 됩니다.

| 접근 범위 | 채팅에서 쓸 수 있는 사람 |
| --- | --- |
| 사이트 소유자만 | 본인만 |
| 특정 부서 | 그 부서 |
| 모든 사용자(외부 제외) | 전사 |

전사 사용을 원하면 사이트 권한에 **모든 사용자(외부 사용자 제외)** 를 편집 권한으로 추가합니다.
사이트 소유자가 직접 할 수 있습니다.
