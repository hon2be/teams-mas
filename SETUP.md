...
**전용 팀을 저장소로 쓸 때 주의** — 팀 사이트는 기본적으로 팀 구성원만 접근합니다.
전사에서 채팅 기능을 쓰게 하려면 둘 중 하나가 필요합니다.

1. 전사를 그 팀에 초대 (인원이 많으면 비현실적)
2. **SharePoint 사이트 권한에 직접 추가** — 사이트 → 사이트 권한 →
   `<사이트명> 구성원` 그룹에 **모든 사용자(외부 사용자 제외)** 추가

2번이 현실적입니다. 팀 멤버십과 무관하게 사이트 접근만 열어줍니다.
